...
def generate_sparql(uris):
    ...
    WHERE {
        # 1. Relaxed Constraints: Don't require rdf:type or rdfs:label
        ?pokemon :hasAttribute ?part .

        # 2. Extract Name from URI (e.g., pk:charizard -> "Charizard")
        BIND(REPLACE(STR(?pokemon), "^.*[#/]", "") AS ?slug)
        BIND(CONCAT(UCASE(SUBSTR(?slug, 1, 1)), SUBSTR(?slug, 2)) AS ?name)
    """

    # Add the reasoning Logic ( * )
    for uri in uris:
        sparql += f"""
        ?part :structuralPartOf* :{uri} .
        """
